...
int main(int argc, char *argv[])
{
    ...
    while (pimple.run(runTime))
    {
        ...
        // Check the cells that have melted (current condition)
        volScalarField alphaMetal =
            mesh.lookupObject<volScalarField>("alpha.metal");
        condition = pos(alphaMetal - 0.5) * pos(epsilon1 - 0.5);

        // MODIFICATION: Implement dual logic for meltHistory and meltTrackID

        // 1. Update original meltHistory (simple cumulative counter)
        meltHistory += condition;

        // 2. Update new meltTrackID (overwrite with latest track number)
